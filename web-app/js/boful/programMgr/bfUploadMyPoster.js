...
function myUploadPosterSuccess(file, serverData) {
    try {
        ...
        if (eval("(" + serverData + ")").success) {
            ...
            if (filePath) {
                $.post(baseUrl + "my/replacPoster", {programId: programId, posterPath: filePath, posterHash: fileHash}, function (data) {
                    if (data.success) {
                        alert("替换完成!");
                    } else {
                        alert("替换失败!")
...
function myUploadVerticalPosterSuccess(file, serverData) {
    try {
        ...
        if (eval("(" + serverData + ")").success) {
            ...
            var programId = $("#programId").val();
            if (filePath) {
                $.post(baseUrl + "my/replaceVerticalPoster", {programId: programId, posterPath: filePath, posterHash: fileHash}, function (data) {
                    if (data.success) {
                        alert("替换完成!");
                    } else {
                        alert("替换失败!")
