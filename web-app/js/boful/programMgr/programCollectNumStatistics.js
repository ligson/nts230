...
$(document).ready(function () {
    ...
    $.post("programCollectNumStatisticsFrom", {
    }, function (data) {
        option.xAxis.categories = data.categories;
        option.series[0].data = data.data;
        $('#programCollectNumStatisticsId').highcharts(option);
    });
    //给今天，本周，本月绑定事件
    $(".program_statistics_content .statistics_time .t_present span").bind("click", function () {
        ...
        $.post("programCollectNumStatisticsFrom", {
            ...
            $('#programCollectNumStatisticsId').highcharts().xAxis[0].setCategories(data.categories);
            $('#programCollectNumStatisticsId').highcharts().series[0].setData(data.data);
        })
    })
    //添加时间控件
