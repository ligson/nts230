...
function onUrlTypeChange(theObj) {
    ...
    if (theObj.value == global.urlType.VIDEO || theObj.value == global.urlType.MOBILE || theObj.value == global.urlType.TABLET || theObj.value == global.urlType.TEXT_LIBRARY || theObj.value == global.urlType.EMBED_PC || theObj.value == global.urlType.DOCUMENT) {
        form1.codeState.disabled = false;
        if (theObj.value == global.urlType.DOCUMENT) {
            form1.codeState.value = Serial.NO_CODE_STATE;
        }
        else {
...
function OnUpFinished() {
    ...
    //不是字幕，富文本等
    if (global.uploadType < UPLOAD_TYPE_SUBTITLE) {
        var url = baseUrl + 'program/dealUpload';
        var params = 'transcodeState=' + transcodeState + '&program.id=' + global.programId + '&urlType=' + urlType + '&codeState=' + form1.codeState.value + '&filePath=' + encodeURIComponent(fileInfo);
        try {
            new Ajax.Updater({success: 'serialList', failure: 'error'}, url, {asynchronous: true, evalScripts: true, parameters: params});
        } catch (e) {
            $.ajax({
                url: url,
                data: params,
                success: function (data) {
                    $("#serialList").empty().append(data)
                },
                error: function (data) {
...
function submitLink() {
    ...
    var url = baseUrl + 'program/dealUpload';
    var params = 'program.id=' + global.programId + '&urlType=' + form1.urlType.value + '&codeState=' + form1.codeState.value + '&filePath=' + encodeURIComponent(fileInfo);
    try {
        new Ajax.Updater({success: 'serialList', failure: 'error'}, url, {asynchronous: true, evalScripts: true, parameters: params});
    } catch (e) {
        $.ajax({
...
function deleteSerial() {
    ...
    var url = baseUrl + 'program/deleteSerial';
    var params = 'program.id=' + global.programId + '&idList=' + idList;
    try {
        new Ajax.Updater({success: 'serialList', failure: 'error'}, url, {asynchronous: true, evalScripts: true, parameters: params});
    } catch (e) {
        $.ajax({
...
function deleteSubtitle(serialId, subtitleId) {
    var url = baseUrl + 'program/deleteSubtitle';
    var params = 'serialId=' + serialId + '&subtitleId=' + subtitleId;
    try {
        new Ajax.Updater({success: 'subtitleList', failure: 'error'}, url, {asynchronous: true, evalScripts: true, parameters: params});
    } catch (e) {
        $.ajax({
            url: url,
            data: params,
            success: function (data) {
                $("#subtitleList").empty().append(data)
            },
            error: function (data) {
                $("#error").empty().append(data)
            }
...
function deleteRelationProgram(rid) {
    ...
    try {
        ...
        $.ajax({
            ...
            data: params,
            success: function (data) {
                $("#relationProgramList").empty().append(data)
            },
            error: function (data) {
                $("#error").empty().append(data)
            }
...
function resizeEditSerialUI() {
    var divObj = document.getElementById("editSerial");
    if (divObj) {
        setDivCenter(divObj);
    }
}
