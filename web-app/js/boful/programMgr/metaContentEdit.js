    ...
    for (j = 0; j < global.tdNum; j++) {
        ...
        if (j == 0) {
            ...
            if (metaObj.dataType == "decorate" || useMetaobj.selfId == global.disciplineId || metaObj.dataType == "link" || metaObj.dataType == "enumeration") {
                if (metaObj.repeatNum == 0)
                    vStr += '&nbsp;&nbsp;<input type="button" class="admin_default_but_blue"  value="增加" onclick="addTr(' + vInd + ')">';
                else
                    vStr += '&nbsp;&nbsp;<input type="button" class="admin_default_but_blue"  value="删除" onclick="delTr(this)">';
            }
...
$(function(){
    $("#closeBtn").click(function(){
        self.location.href = baseUrl + "programMgr/programList";
    });
});