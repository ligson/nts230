...
function deleteConsumer(idList){
    var url=baseUrl+"communityManager/consumerDeleteForForumBoard";
...
function operaCommunityConsumer(idList,stateName,isFlag){
    var url=baseUrl+"communityManager/operaCommunityConsumerForForumBoard";
    var target = null;
    $.ajax({
        url:url,
                ...
                if(idList instanceof Array){
                    for(var i=idList.length-1;i>=0;i--){
                        if(stateName=="userState") {
                            $("#boardGrid").setRowData(idList[i],{userState:isFlag});
                        ...
                        if(stateName=="uploadState") {
                            $("#boardGrid").setRowData(idList[i],{uploadState:isFlag});
                        ...
                        if(stateName=="canDownload") {
                            $("#boardGrid").setRowData(idList[i],{canDownload:isFlag});
                        ...
                        if(stateName=="canComment") {
                            $("#boardGrid").setRowData(idList[i],{canComment:isFlag});
                        ...
                        if(stateName=="notExamine") {
                            $("#boardGrid").setRowData(idList[i],{notExamine:isFlag});
                    ...
                    if(stateName=="userState") {
                        $("#boardGrid").setRowData(idList,{userState:isFlag});
                    ...
                    if(stateName=="uploadState") {
                        $("#boardGrid").setRowData(idList,{uploadState:isFlag});
                    ...
                    if(stateName=="canDownload") {
                        $("#boardGrid").setRowData(idList,{canDownload:isFlag});
                    ...
                    if(stateName=="canComment") {
                        $("#boardGrid").setRowData(idList,{canComment:isFlag});
                    ...
                    if(stateName=="notExamine") {
                        $("#boardGrid").setRowData(idList,{notExamine:isFlag});
...
$(function(){
    $("#boardGrid").jqGrid({
        url:baseUrl+"communityManager/forumBoardAdminList",
        datatype: "json",
        mtype: "POST",
        search: true,
        colNames: ["ID", "社区名", "小组名", "管理员", "管理员昵称","真实姓名"/*, "状态", "上传", "下载","评论","用户审核","操作"*/],
        colModel:[
            { name: "id", width: 50, search: false},
            {name: "communityName", width: 100, search: false},
            {name: "boardName", width: 100, search: true},
    ...
    $("#searchBtn").click(function(){
        ...
        var url = baseUrl + "communityManager/forumBoardAdminList";
        var postData = {
            name: null,
            communityId: null
        };
        ...
        jQuery("#boardGrid").jqGrid("setGridParam", {url: url,page:1,postData:postData}).trigger("reloadGrid");
    })
