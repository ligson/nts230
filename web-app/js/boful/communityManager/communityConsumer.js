...
function operationFormatter(cellvalue, options, rowObject){
    ...
    return "<a class='ui-button searchbtn' onclick='deleteConsumer("+rowObject.id+")'>删除</a>&nbsp;&nbsp;<a class='ui-button searchbtn' id='userStateBtn-"+rowObject.id+"' onclick=\"operaCommunityConsumer("+rowObject.id+",'userState',"+userStateFlg+")\">"+userStateDiv+"</a>&nbsp;&nbsp;<a class='ui-button searchbtn' id= 'uploadBtn-"+rowObject.id+"' onclick=\"operaCommunityConsumer("+rowObject.id+",'uploadState',"+uploadStateFlg+")\">"+uploadStateDiv+"</a>&nbsp;&nbsp;<a class='ui-button searchbtn' id='downloadBtn-"+rowObject.id+"' onclick=\"operaCommunityConsumer("+rowObject.id+",'canDownload',"+canDownloadFlg+")\">"+canDownloadDiv+"</a>&nbsp;&nbsp;<a class='ui-button searchbtn' id='commentBtn-"+rowObject.id+"' onclick=\"operaCommunityConsumer("+rowObject.id+",'canComment',"+canCommentFlg+")\">"+canCommentDiv+"</a>&nbsp;&nbsp;<a class='ui-button searchbtn' id='notExamineBtn-"+rowObject.id+"' onclick=\"operaCommunityConsumer("+rowObject.id+",'notExamine',"+notExamineFlg+")\">"+notExamineDiv+"</a>"
}
function deleteConsumer(idList){
    var url=baseUrl+"communityManager/consumerDeleteForCommunity";
...
function operaCommunityConsumer(idList,stateName,isFlag){
    var url=baseUrl+"communityManager/operaCommunityConsumerForCommunity";
    var target = null;
    $.ajax({
        url:url,
                ...
                if(idList instanceof Array){
                    for(var i=idList.length-1;i>=0;i--){
                        if(stateName=="userState") {
                            $("#consumerGrid").setRowData(idList[i],{userState:isFlag});
                        ...
                        if(stateName=="uploadState") {
                            $("#consumerGrid").setRowData(idList[i],{uploadState:isFlag});
                            target = $("#uploadBtn-"+idList[i]);
                            if(target){
                                if(isFlag == 0) {
                        ...
                        if(stateName=="canDownload") {
                            $("#consumerGrid").setRowData(idList[i],{canDownload:isFlag});
                        ...
                        if(stateName=="canComment") {
                            $("#consumerGrid").setRowData(idList[i],{canComment:isFlag});
                            target = $("#commentBtn-"+idList[i]);
                            if(target){
                                if(isFlag == false || isFlag == "false" || isFlag == 0) {
                        ...
                        if(stateName=="notExamine") {
                            $("#consumerGrid").setRowData(idList[i],{notExamine:isFlag});
                    ...
                    if(stateName=="userState") {
                        $("#consumerGrid").setRowData(idList,{userState:isFlag});
                    ...
                    if(stateName=="uploadState") {
                        $("#consumerGrid").setRowData(idList,{uploadState:isFlag});
                        target = $("#uploadBtn-"+idList);
                        if(target){
                            if(isFlag == 0) {
                    ...
                    if(stateName=="canDownload") {
                        $("#consumerGrid").setRowData(idList,{canDownload:isFlag});
                    ...
                    if(stateName=="canComment") {
                        $("#consumerGrid").setRowData(idList,{canComment:isFlag});
                        target = $("#commentBtn-"+idList);
                        if(target){
                            if(isFlag == false || isFlag == "false" || isFlag == 0) {
                    ...
                    if(stateName=="notExamine") {
                        $("#consumerGrid").setRowData(idList,{notExamine:isFlag});
...
$(function(){
    $("#consumerGrid").jqGrid({
        url:baseUrl+"communityManager/communityConsumerList",
        datatype: "json",
        mtype: "POST",
        search: true,
        colNames: ["ID", "社区名", "小组名", "用户名", "用户昵称","真实姓名"/*, "状态", "上传", "下载","评论","用户审核","操作"*/],
        colModel:[
            { name: "id", width: 50, search: false},
            {name: "communityName", width: 100, search: true},
            {name: "boardName", width: 100, search: false},
    ...
    $("#searchBtn").click(function(){
        ...
        var url = baseUrl + "communityManager/communityConsumerList";
        var postData = {
            name: null,
            communityId: null
        };
