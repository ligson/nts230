...
function show(index) {
    ...
    $("#drag_ctr,#drag_arrow").stop(false, true).animate({ left: index * 30}, 500);
    $("#banner_img>li").eq(curIndex).stop(false, true).fadeOut(time);
    adTxt.eq(curIndex).stop(false, true).fadeIn(500);
    adImg.eq(curIndex).stop(false, true).fadeIn(500);
    ;
    setTimeout(function () {
        ...
        adTxt.eq(index).css({ left: "0", opacity: "0" }).stop(false, true).animate({ left: "10px", opacity: "1" }, time);
        adImg.eq(index).css({ right: "-50", opacity: "0" }).stop(false, true).animate({ right: "10px", opacity: "1" }, time);
    }, 200)
    curIndex = index;
}