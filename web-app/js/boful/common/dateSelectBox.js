...
//var WriteHead=1;
document.write('<iframe style="position:absolute;width:206px;display:none; z-index:2009" name="divCalendar" id="divCalendar" frameborder="0" scrolling="no"></iframe>')
/*头部信息CSS及JS函数*/
...
'#nts.system.domain.Tools {width:204px;background:#5563B0;padding:0px;}'+
'#nts.system.domain.Tools li{padding-bottom:1px;}'+
'#nts.system.domain.Tools a{color:#FFF;}'+
'#nts.system.domain.Tools a:hover{border:1px solid #2E3665;background:#979FCE;}'+
...
'ipt><div style="position: absolute;border:1px solid #6A84C8;height:'+(64+21*Trn)+'px;"><ul>'+
'<li><a href="javascript:parent.Calendar_clean();">清空</a></li>'+
...
'<div id="nts.system.domain.Tools"><ul>'+
...
function Calendar(obj,obj2)
{
	...
	if(Sdate[1].substring(0,1)==0)
		Sdate1 =  Sdate[1].substring(1,2);
	else
		Sdate1 = Sdate[1].substring(0,2);
		 
	Calendar_Show(parseInt(Sdate[0]),parseInt(Sdate1),parseInt(Sdate[2]));
}
function Calendar_GetDate(y,m,d)
{
	var preFixM = '',preFixD = '';
	if (m <=9) preFixM = '0';
	if (d>0 && d<=9) preFixD = '0';
	
	if(!Calendar_obj2||d!=0){
		if(d>0){
			Calendar_obj.value=y+'-'+preFixM+m+'-'+preFixD+d;
		}
		else{
			...
			if(!srcElement){srcElement=objEvent.target;}
			preFixD = '';
			if (srcElement.innerHTML.length < 2) preFixD = '0';
			Calendar_obj.value=y+'-'+preFixM+m+'-'+preFixD+srcElement.innerHTML;
		}
		if (Calendar_obj2 && Calendar_obj2.value < Calendar_obj.value) 
			Calendar_obj2.value = Calendar_obj.value;
	}
	else{
		if(IsLeapYear(y)&&m==2){num=MonthDNum[m]+1;}
		else{num=MonthDNum[m];}
		preFixD = '';
		if (num>0 && num<=9) preFixD = '0';
		Calendar_obj.value=y+'-'+preFixM+m+'-1'; 
		Calendar_obj2.value=y+'-'+preFixM+m+'-'+preFixD+num;
	}
	/*Calendar_hide();*/
}
/*2009-11-9 fanjia update add function of the Calendar_clean*/
function Calendar_clean()
{
	Calendar_obj.value="";
	/*Calendar_hide();*/
}
