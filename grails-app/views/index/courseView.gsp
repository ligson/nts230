<%@ page import="nts.program.domain.RemarkScore; nts.user.domain.Consumer; com.boful.common.date.utils.TimeLengthUtils; java.text.SimpleDateFormat; nts.utils.CTools" contentType="text/html;charset=UTF-8" %>
<html>
<head>
    <title>课程简介</title>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            var playList = ${playLinksNew(serials:videoSerial)};
            ...
            jwplayer("boful_video_player").onPlaylistItem(function (event) {
                ...
                $(".boful_course_name").find("p").empty().append(title);
            });
            ...
            <div class="boful_course_name_box">
                <div class="boful_course_name_title">
                    ...
                    <a class="course_name_second" title="${serial?.program?.name}"
                       href="${createLink(controller: 'index', action: 'courseDetail', params: [programId: serial.program.id])}">&nbsp;${CTools.cutString(serial?.program?.name, 20)}</a>
                </div>
                            ...
                            <div class="overview">
                                <div class="infor_box_lists">
                                    <g:each in="${videoSerial}" var="s">
                                        <p><span class="infor_box_list_icon"></span><a class="infor_box_list_ws"
                                                                                       href="${createLink(controller: 'index', action: 'courseView', params: [serialId: s.id])}">${s.name}</a><span
    ...
    <div class="boful_course_detail_content_about">
        ...
        <p>
            <g:each in="${otherSerial}" var="serial2" status="sta">
                <g:if test="${sta < 5}">
                    <a <g:if test="${sta == 0}">class="boful_course_detail_attachment_word"</g:if>
                       <g:if test="${sta == 1}">class="boful_course_detail_attachment_voice"</g:if>
                       <g:if test="${sta == 2}">class="boful_course_detail_attachment_images"</g:if>
                       <g:if test="${sta == 3}">class="boful_course_detail_attachment_voice"</g:if>
                       <g:if test="${sta == 4}">class="boful_course_detail_attachment_word"</g:if>
...
<g:form name="photoForm" controller="program" action="downloadSharing">
    <input type="hidden" id="programID" name="programId" value="${serial?.programId}">
