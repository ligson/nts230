...
<head>
    ...
    <link type="text/css" rel="stylesheet" href="${resource(dir: 'skin/blue/pc/admin/css', file: 'tabs.css')}"/>
    ...
    <script language="javascript">
        ...
        tabcount = ${cateTopList.size()};//tab.js中的变量,tab条数
    ...
    <div class="x_CaseCenter">
        <g:each in="${remProgramList}" var="program">
            <div class="x_colum">
                ...
                <p class="x_pContent">${CTools.cutString(CTools.htmlToBlank(fieldValue(bean: program, field: 'description')), 32)}</p>
...
<div class="x_case">
    ...
    <div class="menu">
        <ul style="text-align:center">
            <% for (int i = 0; i < 6 && i < cateTopList.size(); i++) { %>
            <% directroyTitle = i == 0 ? "资源排行" : directoryList[i - 1].name %>
            <li style="text-align:center"><a href="#" onmouseover="easytabs('1', '${i}');"
                                             onfocus="easytabs('1', '${i}');" onclick="return false;" title=""
                                             id="tablink${i}"><span class="x_aSpan">${directroyTitle}</span></a>
            </li>
            <% } %>
    ...
    <!--End of the Tabmenu -->
    <div class="x_CaseCenter tabsbg">

        <% for (int i = 0; i < 6 && i < cateTopList.size(); i++) { %>
        <% program = cateTopList[i][0] %>
        <div id="tabcontent${i}" style="display:${i == 0 ? 'block' : 'none'}">
            <!--选项卡S-->
            <div class="x_BannerContent llph x_listDiv ">
                <div class="x_firstLeft">
                    <div class="x_firstImg"><a title="${fieldValue(bean: program, field: 'name')}"
                                               href="${createLink(action: 'showProgram', controller: 'program', params: [id: program?.id])}" target="_blank">
                    ...
                    <div class="x_firstFont">
                        <a title="${fieldValue(bean: program, field: 'name')}"
                           href="${createLink(action: 'showProgram', controller: 'program', params: [id: program?.id])}" target="_blank">${
                                CTools.cutString(fieldValue(bean: program, field: 'name'), 14)}</a> <BR>

                        ${Program.cnField.dateCreated}：<g:formatDate format="yyyy-MM-dd"
                                                                     date="${program?.dateCreated}"/><BR>
                        ${Program.cnField.frequency}：${fieldValue(bean: program, field: 'frequency')}<BR>
                        ${Program.cnField.downloadNum}：${fieldValue(bean: program, field: 'downloadNum')}
                    </div>

                </div>
                <ul>

                    <% for (int j = 1; j < 4 && j < cateTopList[i].size(); j++) { %>
                    <% program = cateTopList[i][j] %>
                    <li>

                        <b class="x_onePic"></b>
                        <span class="x_content_llph"><a title="${fieldValue(bean: program, field: 'name')}"
                                                        href="${createLink(action: 'showProgram', controller: 'program', params: [id: program?.id])}"
                                                        target="_blank">${
            ...
            <div class="x_BannerContent llph x_listDiv" style="border:0;">
                <ul>
                    <% for (int j = 4; j < 12 && j < cateTopList[i].size(); j++) { %>
                    <% program = cateTopList[i][j] %>
                    <li>
                        <b class="x_onePic"></b>
                        <span class="x_content_llph"><a title="${fieldValue(bean: program, field: 'name')}"
        ...
        <!--End Tabcontent 1-->
        <% } %>
        <div class="x_CaseBottom x_casetabsBg"></div>
    </div>
...
<div class="x_rightFirst">
    <span><a href="#">在线帮助</a></span><span>|</span><span><a href="javascript:selectPage(8);">问题反馈</a></span>
...
<div class="x_rightBanner">
    <div class="x_rightBannerTop">
        <div class="x_titleH">
            <span class="x_titleHLeft x_llph">最新资源</span>
            ...
            <ul>
                <g:each in="${channelList}" status="i" var="channel">
                    <li>
                        ...
                        <span class="x_content_llph_courseBcastPlay"><a
                                href="javascript:NetPlay('${channel.channelName.encodeAsJavaScript()}','${channel.bcastAddr.encodeAsJavaScript()}');"
                                onfocus="this.blur()"><img src="${resource(dir: 'images/skin', file: 'real.gif')}"></a></span>
                    </li>
                </g:each>
            </ul>
        ...
        <div class="x_BannerContent">
            <ul>
                <g:each in="${courseBcastList}" status="i" var="courseBcast">
                    <li>
                        <span class="x_content_llph_courseBcast"><g:link controller="index" onfocus="this.blur()"
                                                                         title="${fieldValue(bean: courseBcast, field: 'channel')}"
                                                                         action="showCourseBcast" target="_blank"
                                                                         id="${courseBcast.id}">${CTools.cutString(fieldValue(bean: courseBcast, field: 'channel'), 11)}</g:link></span>
                        <span class="x_content_llph_courseBcastPlay"><a href="#" onclick="play(this.value);"
                                                                        onfocus="this.blur()"
                                                                        value="${courseBcast?.id}"><img
                                    src="${resource(dir: 'images/skin', file: 'play1.jpg')}"></a></span>
                    </li>
                </g:each>
            </ul>
            ...
            <ul>
                <g:each in="${toolsList}" status="i" var="tools">
                    <li>
                        ...
                        <a href="${resource(dir: 'downdir', file: tools.dirName)}"
