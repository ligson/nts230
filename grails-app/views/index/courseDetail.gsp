...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //鼠标移动字体样式更改
            $(".boful_course_content_class_all a").bind("mouseover", function () {
            ...
            $(".boful_course_content_class_all a").bind("mouseout", function () {
                var span = this.children[0].children;
                for (var i = 0; i < span.length; i++) {
                    if (span[i].className == "boful_course_content_number") {
                        span[i].style.color = "";
...
<div class="boful_course_detail_t">
    <h1><a href="${createLink(controller: 'index', action: 'courseList', params: [categoryId: program.programCategory.id])}">${program.programCategory.name}</a><span>${CTools.cutString(program?.name, 20)}</span>
    </h1>
</div>
    ...
    <div class="boful_course_detail_infor">
        <div class="boful_course_detail_infor_left">
            <p class="course_detail_number"><span
                    ...
                    class="number_leng">${program?.serials?.size()}</span>
            </p>
                ...
                <div class="">
                    ...
                    %{-- <p>讲师：<span>${program?.actor}</span></p>
                     ...
                     <p>分类：<span class="boful_course_detail_name">
                         <a href="${createLink(controller: 'index', action: 'courseList', params: [categoryId: program?.programCategory?.id])}">${program?.programCategory?.name}</a>
                     </span></p>--}%
                </div>
            </div>
            <g:if test="${program?.serials?.size() > 0}">
                <div class="detail_infor_join">
                    <a href="${createLink(controller: 'index', action: 'courseView', params: [serialId: querySerialFirstId(program: program)])}"
        ...
        <div class="boful_course_content_class_all">
            <div class="course_content_list_item">
                <g:each in="${videoSerial}" var="serial" status="sta">
                    <a href="${createLink(controller: 'index', action: 'courseView', params: [serialId: serial?.id])}"
                       target="_blank">
            ...
            <div class="page">
                <g:guiPaginate controller="index" action="courseDetail" total="${serialTotal}"
    ...
    <div class="boful_course_detail_content_about">
        ...
        <p>
            <g:each in="${otherSerial}" var="serial" status="sta">
                <g:if test="${sta < 5}">
                    <a <g:if test="${serial.urlType == nts.program.domain.Serial.URL_TYPE_DOCUMENT}">class="boful_course_detail_attachment_word"</g:if>
                       <g:if test="${serial.urlType == nts.program.domain.Serial.URL_TYPE_VIDEO}">class="boful_course_detail_attachment_voice"</g:if>
                       <g:if test="${serial.urlType == nts.program.domain.Serial.URL_TYPE_IMAGE}">class="boful_course_detail_attachment_images"</g:if>
...
<script type="text/javascript">
    function moreRemark(tag) {
        window.location.href = baseUrl + "index/courseView?serialId=" + tag;
    }
</script>
</body>
